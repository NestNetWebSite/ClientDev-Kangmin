...
export default function FileUploadDropzone({ addFiles }: Props) {
    ...
    return (
        <div className={'my-8 flex flex-col'}>
            ...
            <div className={'relative h-64 rounded-xl border border-dashed border-rose-800'} {...getRootProps()}>
                <input {...getInputProps()} />
                <div
                    className={
                ...
                >
                    <MdOutlineFileUpload className={'h-12 w-12 text-rose-800'} />
                    <span className={'font-semibold'}>드로그 앤 드롭으로 파일 추가</span>
                    <span className={'text-sm text-gray-600'}>또는</span>
                    <button
                        type={'button'}
                        className={'rounded-2xl bg-rose-800 px-3 py-2 text-white transition-all hover:bg-rose-900'}
                        onClick={open}
                    >
                        파일 탐색
                    </button>
