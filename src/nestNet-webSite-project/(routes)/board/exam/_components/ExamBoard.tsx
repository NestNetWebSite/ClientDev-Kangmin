...
export default function ExamBoard({ id, year, semester, examType, subject, professor, userName, onBoardClick }: Props) {
    return (
        <article
            className={
                'my-3 flex h-56 cursor-pointer flex-col justify-between rounded-3xl border border-gray-200 bg-white px-7 py-6 shadow-xl transition-all hover:-translate-y-1 hover:shadow-2xl'
            }
            onClick={() => {
                onBoardClick(id);
            }}
            ...
            <div>
                <div className={'mx-1 mb-5 flex justify-between'}>
                    <div className={'flex items-center gap-x-1.5'}>
                        ...
                        <span className={'font text-[0.9rem] font-bold text-gray-500'}>
                            {year}년도 {semester}학기
                        </span>
                    </div>
                    <div>
